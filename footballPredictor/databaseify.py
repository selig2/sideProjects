...
def databaseifyStats():
	...
	cursor.execute("DROP TABLE IF EXISTS NFLStatistics")
...
def winTrendsHTMLToMatrix(htmlFile):
	...
	for fileName in batch:
		...
		for i in range(32):
			...
			# if((fileName[:-9] == "win_Trends_After_A_Win" or fileName[:-9] == "win_Trends_After_A_Loss")  and len(text) == 218 and i == 31):
			# 	break
			# else:
		...
		#print wins
		masterRet.append(sortedWins)
		masterRet.append(sortedMarginOfVictory)
		masterRet.append(sortedATS)

	return tuple(masterRet)

def databaseifyWinTrends():
	connection = sqlite3.connect('footballStats.db') # returns a connection object
	cursor = connection.cursor() #returns a cursor object
	cursor.execute("DROP TABLE IF EXISTS NFLWinTrends")
	cursor.execute("CREATE TABLE NFLWinTrends(uniqueID TEXT, currRecord REAL, currMov REAL, currAts REAL, record2003 REAL, mov2003 REAL, ats2003 REAL, record2004 REAL, mov2004 REAL, ats2004 REAL, record2005 REAL, mov2005 REAL, ats2005 REAL, record2006 REAL, mov2006 REAL, ats2006 REAL, record2007 REAL, mov2007 REAL, ats2007 REAL, record2008 REAL, mov2008 REAL, ats2008 REAL, record2009 REAL, mov2009 REAL, ats2009 REAL, record2010 REAL, mov2010 REAL, ats2010 REAL, record2011 REAL, mov2011 REAL, ats2011 REAL, record2012 REAL, mov2012 REAL, ats2012 REAL, record2013 REAL, mov2013 REAL, ats2013 REAL)")
	
	situationIDs = {"Win_Loss": 1, "As_Home_Team": 6, "As_Away_Team": 7, "Conference_Games": 18, "Non-Conference_Games": 19, "Division_Games": 20, "Non-Division_Games": 21}
	#"After_A_Win": 4,  ADD THESE IN AFTER THEY FIX THEIR SHIT
	#"After_A_Loss": 5, 
	#, "Playoff_Games": 22
	for situation in situationIDs:
		fileName = "win_Trends_" + situation+"2014.html" 
		matrix = winTrendsHTMLToMatrix(fileName)
		list = []
		for j in range(len(matrix[0])): #should be 32 #loop over the rows of our matrix and add them to the table
			#now insert rows into table
			params = (matrix[0][j] + str(situationIDs[situation]), matrix[1][j], matrix[2][j], matrix[3][j], matrix[4][j], \
			matrix[5][j], matrix[6][j], matrix[7][j], matrix[8][j], matrix[9][j], matrix[10][j], matrix[11][j], matrix[12][j], matrix[13][j],\
			matrix[14][j], matrix[15][j], matrix[16][j], matrix[17][j], matrix[18][j], matrix[19][j], matrix[20][j], matrix[21][j], matrix[22][j],\
			matrix[23][j], matrix[24][j], matrix[25][j], matrix[26][j], matrix[27][j], matrix[28][j], matrix[29][j], matrix[30][j], matrix[31][j],\
			matrix[32][j], matrix[33][j], matrix[34][j], matrix[35][j], matrix[36][j])
			list.append(params)
		cursor.executemany("INSERT INTO NFLWinTrends VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", list)
		#print "Database updated for: " + situation
	connection.commit()
	connection.close()	


databaseifyWinTrends()
databaseifyStats()
